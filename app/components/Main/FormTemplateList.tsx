...
import { Search, ChevronDown, ChevronUp, CheckCircle, Sparkles, FileText, Loader2, Plus, FolderOpen, Settings, ChevronRight } from "lucide-react";
import { apiClient } from "@/lib/api/client";
import { Template, TemplateType, Tier, DocumentType, FilterCategory } from "@/lib/api/types";
    ...
    return (
        <div className="py-4 border-b border-gray-200">
            ...
            <div className="text-sm text-gray-600 mb-2">
                <span>{template.tier ? template.tier.charAt(0).toUpperCase() + template.tier.slice(1) : 'Free'}</span>
                {template.created_at && (
                    <span>, {new Date(template.created_at).toLocaleDateString('th-TH', { month: 'long', year: 'numeric' })}</span>
                )}
            </div>
            ...
            <div className="flex items-center gap-1 flex-wrap">
                ...
                >
                    รายละเอียด
                    <ChevronDown className={`w-3 h-3 transition-transform ${showAbstract ? 'rotate-180' : ''}`} />
                </button>
...
function DocumentTypeResultItem({
    ...
    const isVerified = templates.length > 0 && templates.every(t => t.is_verified);
        ...
        <div className="py-4 border-b border-gray-200">
            ...
            >
                <h3 className="text-base font-medium text-[#007398] group-hover:underline leading-snug mb-1">
                    ...
                    <ChevronDown className={`inline-block ml-2 h-4 w-4 text-gray-400 transition-transform ${expanded ? 'rotate-180' : ''}`} />
                </h3>
            </button>

            {/* Meta info */}
            <div className="text-sm text-gray-600 mb-2">
                <span>{firstTemplate?.tier ? firstTemplate.tier.charAt(0).toUpperCase() + firstTemplate.tier.slice(1) : 'Free'}</span>
            </div>
            ...
            <div className="space-y-2">
                {categories.slice(0, 5).map((category) => (
                    <div key={category} className="text-sm">
                        <span className="text-[#007398] hover:underline cursor-pointer font-medium">
                            {category}
                        </span>
                        <div className="text-xs text-gray-500">in เทมเพลต</div>
                    </div>
                ))}
            </div>
        </div>
...
export default function FormTemplateList() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const allTemplates = [
                    ...(templatesResponse.document_types || []).flatMap(dt => dt.templates || []),
    ...
    const templateMatchesFilter = (template: Template) => {
        ...
        for (const category of filterCategories) {
            ...
            const templateValue = (template as unknown as Record<string, unknown>)[fieldName];
    ...
    const hasFilterSelection = Object.values(selectedFilters).some(arr => arr.length > 0);
    ...
    const filteredDocumentTypes = documentTypes
        .map(docType => {
            ...
            const filteredTemplatesInGroup = (docType.templates || []).filter(templateMatchesFilter);
            ...
            if (searchQuery) {
                const docTypeMatchesSearch =
                    ...
                    (docType.name_en || '').toLowerCase().includes(searchQuery.toLowerCase());
        ...
        .filter((dt): dt is (DocumentType & { templates: Template[] }) => dt !== null && (dt.templates?.length || 0) > 0);
    ...
    const totalResults = filteredDocumentTypes.reduce((sum, dt) => sum + (dt.templates?.length || 0), 0) + filteredOrphanTemplates.length;
            ...
            <div className="bg-white border-b border-gray-200">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    ...
                    <div className="max-w-2xl mx-auto">
                        <label className="block text-sm text-gray-600 mb-2">ค้นหาเทมเพลตเอกสาร</label>
                    ...
                    <aside className="w-56 flex-shrink-0">
                        ...
                        <div className="mb-4">
                            ...
                            <button className="text-sm text-[#007398] hover:underline flex items-center gap-1 mt-1">
                                <span className="w-4 h-4 border border-current rounded-full text-xs flex items-center justify-center">!</span>
                                ตั้งการแจ้งเตือน
                            </button>
                        </div>
                        ...
                        <div className="mb-4">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-sm font-semibold text-gray-900">กรองตาม:</h3>
                                {hasActiveFilters && (
                                    <button
                    ...
                    <main className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-end mb-4 pb-3 border-b border-gray-200">
                            <div className="flex items-center gap-2 text-sm text-gray-600">
                                ...
                                <button
                                    ...
                                    className={sortBy === 'relevance' ? 'text-[#007398] font-medium' : 'hover:text-[#007398]'}
                                >
                                    ความเกี่ยวข้อง
                                </button>
                                ...
                                <button
                                    ...
                                    className={sortBy === 'date' ? 'text-[#007398] font-medium' : 'hover:text-[#007398]'}
                                >
                                    วันที่
                                </button>
                            </div>
                        ...
                        {!loading && !error && (
                            <div>
                                ...
                                {/* Empty State */}
                                {sortedDocumentTypes.length === 0 && sortedOrphanTemplates.length === 0 && (
                                    <div className="text-center py-12 bg-white rounded border border-gray-200">
                                        <FolderOpen className="w-12 h-12 text-gray-300 mx-auto mb-4" />
                                        <p className="text-sm text-gray-500 mb-4">
                                            {hasActiveFilters ? 'ไม่พบเทมเพลตที่ตรงกับการค้นหา' : 'ยังไม่มีเทมเพลต'}
                                        </p>
                                        {hasActiveFilters && (
                                            <button
                                                onClick={clearFilters}
